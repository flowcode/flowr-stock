...
class RawMaterialControllerTest extends WebTestCase
{
    public function testCreate()
    {
        $client = static::createClient();
        $crawler = $client->request('GET', '/stock_rawmaterial/');
        ...
        $form = $crawler->filter('form button[type="submit"]')->form(array(
            'rawmaterial[name]' => 'Lorem ipsum dolor sit amet',
            'rawmaterial[stock]' => 42,
            'rawmaterial[enabled]' => true,
                    ));
        $client->submit($form);
    ...
    {
        $client = static::createClient();
        $crawler = $client->request('GET', '/stock_rawmaterial/new');
